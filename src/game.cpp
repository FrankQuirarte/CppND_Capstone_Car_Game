...
void Game::Run(Controller const &controller, Renderer &renderer, std::size_t target_frame_duration) {
  ...
  while (running) {
    ...
    //moves the car and check for collisions
    Update();

    //if the car has crashed the game ends
    if (car.collision == true) { running = false; }

    renderer.RenderBackGround();
    
    renderer.RenderCars(car, enemy1, enemy2);
...
void Game::Update() {
  ...
  //move the enemies
  enemy1.enemyHorizontalMove(score);
  enemy2.enemyHorizontalMove(score);
}
